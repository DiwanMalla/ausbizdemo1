        ...
        <div className="relative">
          <div className="flex overflow-x-auto pb-10 hide-scroll-bar scroll-smooth px-6 lg:px-12 space-x-6">
            {services.map((service, index) => (
              <Card
                key={index}
                className="flex-none w-64 sm:w-80 lg:w-96 bg-card shadow-md rounded-lg transition-all duration-300 transform hover:scale-105 hover:shadow-xl dark:bg-card-dark dark:shadow-md-dark"
              >
                <CardHeader className="flex justify-center items-center p-6 bg-primary/10 dark:bg-primary/20 rounded-t-lg">
          ...
          <div className="absolute top-0 left-0 right-0 bottom-0 bg-gradient-to-r from-transparent via-transparent to-background pointer-events-none"></div>
        </div>
      </div>
    </section>
  );
